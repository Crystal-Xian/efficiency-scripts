...
def _get_fxa_conn():
    return pymysql.connect(cursorclass=pymysql.cursors.DictCursor, **_DB_CONFIGS["sit_fxa"])


class AgentFxaSendVerifier:
    def __init__(
        self,
        bank_req_id: str,
        order_id: str,
    ...
    def _verify_common_trans_out(self) -> bool:
        rec = self._db_query(
            ...
            "sender_remi_id, receive_remi_id, coin_id, amount, direction, tx_id "
            "FROM message_transaction_record "
            "WHERE bank_req_id = %s AND business_type = 'COMMON_TRANS_OUT' LIMIT 1",
            (self.bank_req_id,)
        )
        if self.report:
    ...
    def verify_all(self, timeout: int = 180) -> bool:
        try:
            self.log_step(0, "FXA 发送开始", "RUNNING",
            ...
            self.log_step(4, "FXA 发送最终结果", "PASS", "全部核查通过", None)
            return True
        finally:
            self.close_conn()
