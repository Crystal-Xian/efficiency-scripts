...
def _get_conn(db_key: str):
    return pymysql.connect(cursorclass=pymysql.cursors.DictCursor, **_DB_CONFIGS[db_key])


class AgentFxcSendVerifier:
    def __init__(
        self,
        bank_req_id: str,
        order_id: str,
    ...
    def _verify_es_update_onchain_status(self) -> bool:
        ...
        pod_key = self.sender_bic[:3].lower()
        while time.time() < deadline:
            ...
            for h in (pod_hits or hits):
                msg = str(h.get("message", ""))
                if "ACK" in msg and "COMMON_TRANS_OUT" in msg:
                    self.log_step(4, "ES: updateOnchainStatus", "PASS",
                                  f"找到 ACK 回调, pod={h.get('pod_name','')}", h)
                    return True
                if "NAK" in msg:
                    self.log_step(4, "ES: updateOnchainStatus", "FAIL",
                                  f"收到 NAK, pod={h.get('pod_name','')}", h)
                    return False
            time.sleep(self.es_interval_s)
        self.log_step(4, "ES: updateOnchainStatus", "TIMEOUT",
                      f"超时 {self.es_timeout_s}s 未找到回调日志", None)
        return False

    def _verify_es_inward_message_receive(self) -> bool:
        deadline = time.time() + self.es_timeout_s
        pod_key = self.receiver_bic[:3].lower()
        while time.time() < deadline:
            hits, _ = self.es_checker.search_once(
                order_id=self.bank_req_id,
            ...
            if pod_hits:
                ...
                if "COMMON_TRANS_IN" in msg:
                    self.log_step(5, f"ES: inwardMessageReceive ({self.receiver_bic})", "PASS",
                                  f"找到汇入通知, pod={hit.get('pod_name','')}", hit)
                    return True
            time.sleep(self.es_interval_s)
        self.log_step(5, f"ES: inwardMessageReceive ({self.receiver_bic})", "TIMEOUT",
                      f"超时 {self.es_timeout_s}s 未找到汇入通知日志", None)
        return False

    def verify_all(self, timeout: int = 180) -> bool:
        try:
            self.log_step(0, f"{self.sender_bic} 发送开始", "RUNNING",
            ...
            if not self._verify_es_inward_message_receive():
                return False

            self.log_step(6, f"{self.sender_bic} 发送最终结果", "PASS", "全部核查通过", None)
            return True
        finally:
            self.close_conn()


class AgentFxdSendVerifier(AgentFxcSendVerifier):
    def __init__(self, bank_req_id: str, order_id: str, amount: float, coin_id: str,
                 sender_bic: str, receiver_bic: str,
                 report: Optional["UnifiedReportWriter"] = None):
        super().__init__(bank_req_id, order_id, amount, coin_id,
                         sender_bic, receiver_bic, report, db_key="sit_fxd")
